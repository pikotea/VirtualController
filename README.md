...
3. このツールの画面上部のコントローラーUIで、ボタンや方向キーなどを操作するとゲーム内で正常に接続されたアナウンスが出ます。
<img src="docs/images/setting_03_connect2.png" alt="接続" width="400">

4. あとは適当にマクロファイルを作成し、マクロを選択して「再生」ボタンを押すとマクロが実行されます。ランダム再生したい場合は複数選択し、「ランダム」にチェックを入れて「再生」
ボタンを押してください。
<img src="docs/images/setting_05_play.png" alt="再生" width="400">

5. ボタンの割り当てはデフォルトで問題はないはずですが、必要があればこのツールの画面上部のコントローラーUIで、「BACK」ボタンを押して**2P側**の「CONTROLLER SETTINGS」を開きボタンを割り当ててください。
...
- キーは以下の形式で指定可能（大文字・小文字区別なし）
  - ボタン: A, B, X, Y, RT, LT, RB, LB
  - ボタン: LP, LK, MP, MK, HP, HKでも可です（A=LP, B=MP, X=HP, Y=LK, RB=MK, RT=HK）
...
また、`TRIGGERWAIT:` でトリガー待機中にしゃがみにしておくなど待機中のポーズなども指定できます。
...
上記のマクロに以下のマクロを一緒に再生すると、位置リセット後に自動で前ジャンプして位置を入れ替え、自分がジャンプすると空対空が飛んできて裏周りしてきます。色々応用が効くと思うので頑張ってください。
...
TRIGGER機能の注意事項
- トリガーとなるキーを押してからマクロが再生されるまでに1F～3F程度の誤差が出る可能性があります
- 記録設定を完了していないとTRIGGER指定は無視されます（警告も出ません）
- TRIGGER指定ありのマクロとなしのマクロを同時に複数選択するとTRIGGER指定ありのマクロのみが再生されます
- TRIGGER指定ありのマクロを同時に複数選択した状態で、同じトリガー条件が同時に成立するとランダムで再生されます
- TRIGGERWAITが指定されているマクロを複数選択した場合、最初のTRIGGERWAITが優先されます
- その他
  - 位置リセットをトリガーにするとゲーム側の切り替え処理が終わるのを待つ必要があるのでマクロの先頭に待ちフレームを追加しましょう
  - 攻撃ボタンをトリガーにしてガード後行動をさせる場合は、マクロの先頭にガードを入れましょう

---

## マクロファイル仕様（ランダム確率調整機能）

ランダム再生する際に特定のマクロの確率を調整したい場合、
マクロの先頭にコメントで `RANDOM:`を追加してキーを指定します。
以下の2つのマクロをランダム再生するとLPが3倍の確率で選択されます。
...
LPは、3/4の確率で選択され、LKは1/4の確率で選択されます。
RANDOMを明示的に指定しない場合は、デフォルトで1として扱われます。

また、`TRIGGER:` と併用することも可能です。
...
こちらの大Pに大して指定の確率でランダム再生されます。

---

## フィードバック

Xでハッシュタグ [`#仮想コマコン`](https://x.com/search?q=%23%E4%BB%AE%E6%83%B3%E3%82%B3%E3%83%9E%E3%82%B3%E3%83%B3&src=typed_query) で動作確認や不具合の報告、要望、感想などをポストしてくれればと思います。
（対応を必ずお約束するものではありませんのであしからず）
...
- PCが高負荷時にはマクロの再生に遅延が発生することがあります。
